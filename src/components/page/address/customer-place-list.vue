    ...
    <div class="table" v-loading="$root.showLoadingIcon">
        <div class="crumbs">
            <el-breadcrumb separator="/">
				<!-- TODO 修改下图标-->
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 用户地址列表</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <el-table :data="data" border class="table">
                <el-table-column prop="index" label="序号" sortable  align="center" width="140"></el-table-column>
                <el-table-column prop="province" label="省" align="center" width="200"></el-table-column>
...
<script>
    export default {
        ...
        computed: {
            data() {
				var vue = this;
                return vue.customerPlaces.map((customerPlace,index) => {
                   return {
					   index : index + 1,
					   id : customerPlace.id,
					   province: customerPlace.province,
					   city: customerPlace.city,
					   district: customerPlace.district,
					   street: customerPlace.street,
        ...
        methods: {
            getCustomerPlacesList() {
				var vue = this;
				vue.$jsonAxios.get(vue.url).then(function(response){
					//这里只能说明返回的状态码是以2开头的.
					vue.$util.getSuccessHandler(response,vue,"customerPlaces");
				}).catch(function(err){
