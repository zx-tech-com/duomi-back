    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
				<!-- TODO 修改下图标-->
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 轮播图列表</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button type="primary" icon="delete" class="handle-del mr10" @click="handleAdd">新增轮播图</el-button>
            </div>
...
<script>
    export default {
        ...
        computed: {
            data() {
				var vue = this;
                return vue.banners.map((banner,index) => {
                   return {
					   index : index + 1,
					   id : banner.id,
					   imgurl : vue.$util.getFullAttachmentUrl(banner.imgurl),
        ...
        methods: {
            getRawBannerList() {
				...
				vue.$jsonAxios.get(vue.url).then(function(response){
					//这里只能说明返回的状态码是以2开头的.
					vue.$util.getSuccessHandler(response,vue,"banners");
				}).catch(function(err){
